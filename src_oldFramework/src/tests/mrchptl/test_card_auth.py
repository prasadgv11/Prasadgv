...
@allure.feature("To Verify the ADPAy EN Auth Transactions for Versions 1.0.1, 1.0.6 & 1.0.7")
def test_ADPAY_cardAuth(page, data):
    demo = demoPage(page)
    ...
    demo.click_buy()
    auth.verify_payment_status()

# Arabic changes for Auth  Transaction
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the ADPAy AR Auth Transactions for Versions 1.0.1, 1.0.6 & 1.0.7")
def test_ADPAY_cardAuth_AR(page, data):
    demo = demoPage(page)
    auth=PurchasePage(page)
    demo.open_base()
    demo.go_authorization()
