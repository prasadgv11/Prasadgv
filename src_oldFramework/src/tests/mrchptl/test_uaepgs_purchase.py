...
@allure.feature("To Verify the ADPAy UAEPGS Purchase Transactions for Versions 1.0.1, 1.0.6 & 1.0.7")
   
def test_ADPAY_UAEPGS_Purchase(page, data):
    demo = demoPage(page)
    ...
    demo.click_buy()
    UaepgsPurchase.verify_payment_status()

# Arabic changes for UAEPGS Purchase Transaction
...
def test_ADPAY_UAEPGS_Purchase_AR(page, data):
    demo = demoPage(page)
    UaepgsPurchase= PurchasePage(page)
    demo.open_base()
    demo.go_purchase()
