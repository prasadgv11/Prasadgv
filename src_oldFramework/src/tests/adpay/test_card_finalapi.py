...
@allure.feature("To verify Finalization API completion Transactions for ADPAY")
def test_adpay_Finalapi_auth(page, data):
    demo = demoPage(page)
...
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To verify Finalization API completion Transactions for ADPAY")
def test_adpay_Finalapi_Purchase(page, data):
    demo = demoPage(page)
    Finalizationapi=PurchasePage(page)
    demo.open_base()
    demo.go_purchase()
