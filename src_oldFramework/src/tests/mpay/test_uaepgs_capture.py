...
@allure.feature("To Verify the MPAy UAEPGS EN Capture Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_uaepgs_Capture(page, data):
    demo = demoPage(page)
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the MPAy UAEPGS AR Capture Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_uaepgs_Capture_AR(page, data):
    demo = demoPage(page)
    uaepgsauth=PurchasePage(page)
    demo.open_base()
    demo.go_authorization()
