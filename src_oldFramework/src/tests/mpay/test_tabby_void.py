...
@allure.feature("To Verify the MPAy Tabby EN Void Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Void_Tabby(page, data):
    demo = demoPage(page)
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the MPAy Tabby AR Void Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Void_Tabby_AR(page, data):
    demo = demoPage(page)
    tabbypurchase=PurchasePage(page)
    demo.open_base()
    demo.go_purchase()
