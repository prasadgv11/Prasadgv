...
@allure.feature("To Verify the MPAy Tabby EN Purchase Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_purchase_Tabby(page, data):
    demo = demoPage(page)
    ...
    demo.click_buy()
    tabbypurchase.verify_payment_status_Mpay()
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the MPAy Tabby AR Purchase Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Purchase_Tabby_AR(page, data):
    demo = demoPage(page)
    tabbypurchase=PurchasePage(page)
    demo.open_base()
    demo.go_purchase()
