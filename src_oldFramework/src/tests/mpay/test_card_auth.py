...
@allure.feature("To Verify the MPay EN Auth Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Auth_card(page, data):
    demo = demoPage(page)
...
#    auth.fill_receipt_email(data["Receipt_email"])
    ...
    demo.click_buy()
    # auth.verify_payment_status_Mpay()
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the MPay AR Auth Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Auth_card_AR(page, data):
    demo = demoPage(page)
    auth=PurchasePage(page)
    demo.open_base()
    demo.go_authorization()
