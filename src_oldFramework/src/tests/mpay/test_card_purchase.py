...
@allure.feature("To Verify the MPay EN Purchase Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_purchase_card(page, data):
    demo = demoPage(page)
    ...
    demo.click_buy()
    purchase.click_charity_checkbox(charity_amt=data["charity_amount"])
    ...
    demo.click_buy()
    purchase.verify_payment_status_Mpay()
...
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the MPay EN Purchase Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_purchase_card_AR(page, data):
    demo = demoPage(page)
    purchase=PurchasePage(page)
    demo.open_base()
    demo.go_purchase()
    ...
    purchase.select_MM(data["Exp_Month"])
    purchase.select_YYYY(data["Exp_Year"])
    purchase.click_charity1_checkbox_AR(charity_amt=data["charity_amount"]) 
    purchase.click_charity2_checkbox_AR(charity_amt=data["charity_amount"])
    purchase.click_charity3_checkbox_AR(charity_amt=data["charity_amount"])
    purchase.fill_receipt_email(data["Receipt_email"])
    purchase.click_Mpay()
    purchase.verify_payment_Mpay()
    purchase.capture_payment_id()
