...
@allure.feature("To verify Mpay Purchase page validations")
def test_DemoPagae_Validations(page, data):
    demo = demoPage(page)
...
purchase_data = ExcelUtil.get_sheet("Datasheet", "Auth_page")
print("Loaded purchase_data:", purchase_data)

@pytest.mark.regression_sanity
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To verify MPAY Auth  page validations")
def test_DemoPagae_Validations(page, data):
    demo = demoPage(page)
    demopage=PurchasePage(page)
    demo.open_base()
