...
@allure.feature("To Verify the MPAy UAEPGS EN Refund Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_uaepgs_Refund(page, data):
    demo = demoPage(page)
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the MPAy UAEPGS AR Refund Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_uaepgs_Refund_AR(page, data):
    demo = demoPage(page)
    uaepgspurchase=PurchasePage(page)
    demo.open_base()
    demo.go_purchase()
