...
@allure.feature("To Verify the MPay EN Void Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Void_card(page, data):
    demo = demoPage(page)
    ...
    purchase.click_charity_checkbox(
        charity_amt=data["charity_amount"]
    )
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the MPay AR Void Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Void_card_AR(page, data):
    demo = demoPage(page)
    purchase=PurchasePage(page)
    demo.open_base()
    demo.go_purchase()
