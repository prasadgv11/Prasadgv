...
@allure.feature("To Verify the MPAy UAEPGS EN Void Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Void_uaepgs(page, data):
    demo = demoPage(page)
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To Verify the MPAy UAEPGS AR Void Transactions for Versions 1.0.1 & 1.0.5")
def test_MPAY_Void_uaepgs_AR(page, data):
    demo = demoPage(page)
    uaepgspurchase=PurchasePage(page)
    demo.open_base()
    demo.go_purchase()
