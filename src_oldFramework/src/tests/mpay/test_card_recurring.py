...
purchase_data = ExcelUtil.get_sheet("Datasheet", "mpay_Recurring_txn")
print("Loaded purchase_data:", purchase_data)

@pytest.mark.regression_sanity
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To verify Mpay Recurring Register & Recurring payment")
def test_Mpay_Recurring_Register_Payment(page, data):
    demo = demoPage(page)
    ...
    demo.select_recurring_project()
    demo.click_buy()
    recurring_reg.fill_recurringcard_details(
        name=data["card_name"],
        number=data["card_number"],
    )
    recurring_reg.select_Month_recurring(data["Exp_Month"])
    recurring_reg.select_Year_recurring(data["Exp_Year"])
...
@pytest.mark.sanity
@pytest.mark.parametrize("data", purchase_data)
@allure.feature("To verify Mpay Recurring list, Recurring Register & Recurring payment")
def test_Mpay_Listcard_Recurring_Register(page, data):
    demo = demoPage(page)
    recurring_reg=PurchasePage(page)
    demo.open_base()
