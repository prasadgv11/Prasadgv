...
class TestFabMpayTransactions(object):
        def test_mpay_card_purchase(self, page, data):
            ...
            transactions.fill_mpay_receipt_email(BANK_NAME, data)
            if data.get("Language") == "AR":
                transactions.select_mpay_arabic_charities(BANK_NAME, data.get("Charity Amount"))
