...
class TestFabMpayTabbySupportingTransactions(object):
        def test_mpay_tabby_capture(self, page, data):
            tx = _mpay_tabby_payment(page, data, "authorization")
            tx.capture_payment_id(BANK_NAME)
            tx.go_back_home(BANK_NAME)
            tx.open_transaction_portal(BANK_NAME)
            tx.go_to_transaction(BANK_NAME, "capture")
            tx.fill_capture_form(BANK_NAME, data)
